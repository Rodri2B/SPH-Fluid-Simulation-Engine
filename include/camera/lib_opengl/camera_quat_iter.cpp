...
Camera::Camera(unsigned int WindowWidth, unsigned int WindowHeight,glm::vec3 Pos, glm::vec3 up, float yaw, float pitch) : m_target(glm::vec3(0.0f, 0.0f, -1.0f)), MovementSpeed(SPEED), MouseSensitivity(SENSITIVITY), Zoom(ZOOM)
{
    ...
    m_AngleH_old = m_AngleH;
    m_AngleV_old = m_AngleV;
...
void Camera::OnRender(float deltaTime)
{
    ...
    float delta_m_AngleV = m_AngleV - m_AngleV_old;
    float delta_m_AngleH = m_AngleH - m_AngleH_old;

    m_AngleV = m_AngleV_old;
    m_AngleH = m_AngleH_old;

    delta_m_AngleV /= static_cast<float>(iterations);
    delta_m_AngleH /= static_cast<float>(iterations);

    displace[0] /= static_cast<float>(iterations);
    displace[1] /= static_cast<float>(iterations);
    displace[2] /= static_cast<float>(iterations);

    for(unsigned int i = 0; i < iterations; i++){
        m_AngleV += delta_m_AngleV;
        m_AngleH += delta_m_AngleH;
        Update();
        m_pos += m_target*displace[0];
